  ...
  const getRewardDescription = () => {
    ...
    switch (rewardType) {
      ...
      case "flat":
        ...
        return (
          <Text style={styles.descriptionText}>
            Earn {flatPoints} points per transaction
          </Text>
        );

      case "slab":
        if (
          Array.isArray(rewards.slab_rules) &&
          rewards.slab_rules.length > 0
        ) {
          return rewards.slab_rules.map((rule: any, index: number) => {
            ...
            return (
              <Text key={index} style={styles.slabRuleText}>
                • Earn {rule.points}pts on spending ₹{rule.min}{" "}
                {isLast ? "and more" : `to ₹${rule.max}`}
              </Text>
            );
          });
        }
      ...
      default:
        return (
          <Text style={styles.descriptionText}>
            Earn {rewardPoints} points per transaction
          </Text>
        );
    }
  };
    ...
    <Surface style={[styles.card, { backgroundColor: theme.dark ? theme.colors.backdrop : theme.colors.surfaceVariant }]}>
      ...
      {expanded && (
        <>
          ...
          <View style={styles.rewardContent}>{getRewardDescription()}</View>
        </>
      )}
    </Surface>
  );
