...
export default function RewardsSettings() {
  ...
  const upiFromProfile: string[] =
    user?.user?.seller_profile?.rewards.upi_ids || [];
  ...
  const isDirty = useMemo(() => {
    ...
    const rewardsChanges =
      offers.length !== initial.offers.length ||
      offers.some(
        (s, idx) =>
  ...
  const buildNumericSlabs = (): {
    ...
    for (let i = 0; i < slabRules.length; i++) {
      ...
      if (!max || max <= 0 || !pts || pts <= 0) {
        throw new Error(
          "Please fill valid max amount and points for all slabs."
        );
      }
      if (max <= lastMax) {
        throw new Error(
          "Each slab's max amount must be greater than the previous slab."
        );
      }
  ...
  const handleSave = async () => {
    ...
    if (rewardType === "slab") {
      try {
        ...
        return Alert.alert(
          "Validation",
          e.message || "Invalid slab configuration."
        );
      }
    }
    ...
    try {
      ...
      console.error("Save Rewards Settings Error:", err?.response || err);
      Alert.alert(
        "Error",
  ...
  const updateOffer = (
    index: number,
    field: "reward_points" | "reward_description" | "reward_name",
    value: string
  ) => {
          ...
          {!isEditing ? (
            <View>
              ...
              <View style={{ marginTop: 12 }}>
                ...
                {upiIds.length === 0 ? (
                  ...
                  <View style={styles.chipRow}>
                    {upiIds.map((id) => (
                      <Chip key={id} style={styles.chip} disabled>
                        {id}
                      </Chip>
                    ))}
                  </View>
              ...
              <HelperText type="info" style={{ color: theme.colors.onSurface }}>
                Customize how customers earn and redeem rewards, and link your
                UPI IDs for payment-based rewards.
              </HelperText>
            </View>
          ) : (
            /* EDIT MODE */
            <View>
              ...
              {rewardType === "percentage" && (
                <>
                  ...
                  <HelperText
                    ...
                    style={{ color: theme.colors.onSurface }}
                  >
                    You must generate dynamic qr everytime if user paying
                    through cash, card or scan payment qr from different app.
                  </HelperText>
                </>
              )}
                  ...
                  return (
                    ...
                    >
                      <TextInput
                        ...
                        onChangeText={(text) =>
                          updateOffer(index, "reward_points", text)
                        }
                        mode="outlined"
                        label="Reward points"
                        style={[
                      ...
                      <TextInput
                        value={o.reward_name}
                        onChangeText={(text) =>
                          updateOffer(index, "reward_name", text)
                        }
                        mode="outlined"
                        label="Reward Name"
                        style={[
                      ...
                      <TextInput
                        ...
                        onChangeText={(text) =>
                          updateOffer(index, "reward_description", text)
                        }
                        mode="outlined"
              ...
              <View style={{ marginTop: 16 }}>
                ...
                {upiError && <HelperText type="error">{upiError}</HelperText>}
                <HelperText
                  type="info"
                  style={{ color: theme.colors.onSurface }}
                >
                  Example: <Text style={{ fontWeight: "600" }}>myshop@upi</Text>
                  . Only letters, numbers, dot (.) and dash (-) are allowed.
                </HelperText>
              </View>
            </View>
          )}
