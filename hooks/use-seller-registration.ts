...
export const useSellerRegistration = () => {
    ...
    const [slabRules, setSlabRules] = useState<{ min: number; max: string; points: string }[]>(
        [{ min: 0, max: "", points: "" }]
    );
    ...
    const handleRegister = async () => {
        ...
        try {
            const payload = {
                ...
                slabRules: formData.rewardType === "slab"
                    ? slabRules
            ...
            Alert.alert('Registration Success', 'Please login with registered email id and password to continue.');
